...
fn main() {
    env::args().skip(1).for_each(|arg| match fs::read(&arg) {
...
fn generate_ast(mut slice: &str) -> String {
    ...
    while pos < slice.len() {
        ...
        if let Some(index) = slice[pos..].find("// impl Visitor<").map(|i| i + pos) {
            if let Some(post_type) = slice[index..].find("> for").map(|i| i + index) {
                if let Some(post_struct) = slice[post_type..].find(':').map(|i| i + post_type) {
                    if let Some(end) = slice[post_struct..].find(';').map(|i| i + post_struct) {
                        visitors.push((
                            &slice[index + 16..post_type].trim(),
    ...
    while pos < slice.len() {
        // TODO: merge if let chaining becomes stable
        if let Some(index) = slice[pos..].find("// attr").map(|i| i + pos) {
            if let Some(post_name) = slice[index..].find(":").map(|i| i + index) {
                if let Some(end) = slice[post_name..].find(";").map(|i| i + post_name) {
                    attrs.push((
                        &slice[index + 7..post_name].trim(),
    ...
    while pos < slice.len() {
        // TODO: merge if let chaining becomes stable
        if let Some(index) = slice[pos..].find("// cast:").map(|i| i + pos) {
            if let Some(end) = slice[index..].find(";").map(|i| i + index) {
    ...
    while pos < slice.len() {
        // TODO: merge if let chaining becomes stable
        if let Some(index) = slice[pos..].find("// type").map(|i| i + pos) {
            if let Some(post_type) = slice[index..].find(":").map(|i| i + index) {
                if let Some(end) = slice[post_type..].find(";").map(|i| i + post_type) {
                    type_fields.push((
                        &slice[index + 7..post_type].trim(),
    ...
    type_fields.iter().for_each(|tuple| {
        ...
        let mut argsn = main_attrs.to_string().replace(";", ",");
        argsn.push_str("\tpub ");
        argsn.push_str(&args.to_string().replace(", ", ",\n\tpub "));
        text.push_str(
            &TYPE_CODE
                .to_string()
                .replace("TYPE", kind)
